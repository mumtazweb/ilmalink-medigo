...
export default function GeoCountrySection({
  ...
  const summary = `MBBS in ${country.countryName} should be shortlisted only after checking eligibility, NEET status, university recognition, course duration, WDOMS listing, medium of instruction and licensing pathway. ILMALINK MEDIGO's country dataset lists ${country.wdomsCount} WDOMS medical school entries for ${country.countryName}. ${country.eligibilitySummary} ${country.licensingNote}`;
    ...
    <section className="overflow-x-hidden bg-[#F8FAFC] px-4 py-12 text-slate-950 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto grid max-w-7xl gap-6">
        <GeoAnswerBox
          ...
          sourceNote="WDOMS counts are maintained in ILMALINK MEDIGO's country dataset for student shortlisting. Students should verify each university name, recognition status and licensing pathway before admission."
        />
